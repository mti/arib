...
def translate(text, from_language="ja", to_language="en", client_id="", secret_key=""):
    if not secret_key:
        raise Exception(
            "No Microsoft Azure secret key provided on bing.translate call."
        )
    ...
    oauth_junk = json.loads(
        requests.post(oauth_url, data=urllib.parse.urlencode(args)).content
    )
    translation_args = {
    ...
    translation_result = requests.get(
        translation_url + urllib.parse.urlencode(translation_args), headers=headers
    )
...
def main():
    ...
    parser.add_argument(
        "text", help="Input filename (MPEG2 Elmentary Stream)", type=str
    )
    parser.add_argument(
        "-k",
        "--secret_key",
        help="Windows secret key for bing translate API.",
        type=str,
        default="",
    )
